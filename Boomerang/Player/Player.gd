...
export var acceleration_default: = Vector2(8.0, 1000.0)	#Acceleration & gravity
...
func calculate_velocity(delta: float) -> Vector2:
	...
	#HORIZONTAL
	new_velocity.x = lerp(new_velocity.x, direction.x * max_speed.x, acceleration.x * delta)
